...
const Hero = () => {
  ...
  return (
    <section id="hero" className="relative min-h-[600px] md:min-h-screen flex items-start md:items-center overflow-hidden px-5 sm:px-10 lg:px-20 pt-20 sm:pt-28 md:pt-0">
      ...
      <div className="relative z-10 w-full max-w-[1440px] mx-auto asymmetric-grid items-center gap-8 lg:gap-16">
        {/* Hero Content */}
        <div className="space-y-6 lg:space-y-12">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-surface-container-high border border-primary/20 backdrop-blur-xl">
          ...
          <div className="space-y-4 lg:space-y-6">
            <div className="space-y-1 lg:space-y-2">
              <span className="text-base md:text-xl lg:text-2xl font-mono text-white/60 tracking-[0.2em] uppercase">{config.name}</span>
            ...
            <p className="text-lg lg:text-2xl font-light text-on-surface-variant max-w-2xl leading-relaxed border-l-2 border-primary/20 pl-6 lg:pl-8">
              {config.hero_tagline}
            </p>
          </div>
          <div className="flex flex-wrap gap-2 md:gap-4">
            {config.skills.map((skill) => (
              <span key={skill} className="px-3 py-1.5 md:px-5 md:py-2 rounded-lg bg-surface-container-low border border-outline-variant/10 text-on-surface-variant text-[10px] md:text-[11px] font-mono uppercase tracking-widest hover:border-primary/40 hover:text-primary transition-all cursor-default">
                {skill}
              </span>
            ))}
          </div>
